...
from app.models import Game, Play, PlayAdvancedStat, PlayCoverageStat, PlayDriveContext, PlayQbStat
...
def ingest_current_season_plays(db: Session, season: int) -> int:
    ...
    db.query(PlayQbStat).filter(
        PlayQbStat.season == season, PlayQbStat.game_id.in_(final_game_ids)
    ).delete(synchronize_session=False)
